...
def export_airflow_connections_users(client_pod_name,client_container_name,client_name):
    ...
    subprocess.run(["docker", "exec", client_container_name , "airflow", "connections", "export", f"/tmp/{client_name}_connections.json"])

    # Copy the connections file from the container to the local machine
    subprocess.run(["docker", "cp", f"{client_container_name}:/tmp/{client_name}_connections.json", f"./exports_files/{client_name}_connections.json"])
    ...
    subprocess.run(["docker", "exec", client_container_name, "airflow", "users", "export","-v", f"/tmp/{client_name}_users.json"])

    # Copy the users file from the container to the local machine
    subprocess.run(["docker", "cp", f"{client_container_name}:/tmp/{client_name}_users.json", f"./exports_files/{client_name}_users.json"])
