...
def import_airflow_connections_users(client_pod_name,client_container_name,client_name):
    ...
    subprocess.run(["docker", "cp", f"./exports_files/{client_name}_connections.json", f"{client_container_name}:/tmp/{client_name}_connections.json"])

    # Import connections from the file in the container
    subprocess.run(["docker", "exec", client_container_name, "airflow", "connections", "import", f"/tmp/{client_name}_connections.json"])
    ...
    subprocess.run(["docker", "cp", f"./exports_files/{client_name}_users.json", f"{client_container_name}:/tmp/{client_name}_users.json"])

    # Import users from the file in the container
    subprocess.run(["docker", "exec", client_container_name, "airflow", "users", "import", f"/tmp/{client_name}_users.json"])
